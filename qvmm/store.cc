...
static char *id __attribute__((unused))="$Id: store.cc,v 1.7 2002/04/15 09:29:11 coldstore Exp $";
...
struct Branch
{
  ...
  void check() {
    //assert(((int)alloc) <= 1);	// HACK - don't accept allocators
  }
...
public:
  ...
  Node()
    ...
    size_t getSize() {
        assert(node_size <= branches); return node_size;
    }
    void setSize(size_t s);
    size_t &refSize() {return node_size;}

    void* operator new( size_t size);
    void  operator delete( void* ptr);

    bool operator < (Node &n) {
        return branch[node_size-1] < n.branch[0];
    }
...
void Node::check()
{
    ...
    assert(node_size && (node_size <= branches));
    if (node_size > 1)
    ...
    if (!Mmap::check(this)) {
      fault = "mmap";
    } else if (!node_size || (node_size > branches)) {
      fault = "size";
      index = node_size;
    } else if (node_size > 1) {
      for (size_t i = 0; i < (node_size - 2) && !fault; i++) {
    ...
    if (fault) {
      cerr << "Node::check " << fault << ' ' << index << ' ' << branches << '\n';
      assert (!fault);
      throw fault;
    }
#endif
...
struct Found
{
    ...
    /** Nodes - found nodes and offsets within nodes
   */
    struct Nodes {
        Node*  node;
        size_t index;
  ...
  Found(const void *_alloc, const void *_addr)
    : alloc(_alloc),
      addr(_addr),
      node() {}

  /** construct a BTree query from an Edit
   */
  Found(const void *_alloc, const void *_addr, Edit *edit)
    : alloc(_alloc),
      addr(_addr),
      node() {
    node[0].node = edit->branch[0].value.internal;
    if (edit->size == 1)
      node[1].node = 0;
    else
      node[1].node = edit->branch[1].value.internal;
    BTree::lookup(this);
    check();
  }
...
ostream & operator <<(ostream &out, Found::Nodes &n)
{
  out << "node " << n.node << '@' << n.index << '\n';
  return out;
}
...
{
  if (node[0].node) {
    //cerr << "Found::check 0" << node[0] << '\n';
    node[0].node->check();
  }
  if (node[1].node) {
    //cerr << "Found::check 1" << node[1] << '\n';
    node[1].node->check();
  }
}
...
DEBLOG(int nProfileLookupCount = 0);
...
void BTree::lookup( Found* state)
{
    ...
    for (low=0,high=node->getSize(); high>low+1; ) {
        mid = (high + low) / 2;
        Branch *branch = node->branch + mid;
        if (state->alloc < branch->alloc) high = mid;
        else if (state->alloc > branch->alloc) low = mid;
        else if (state->addr < branch->addr) high = mid;
        else low = mid;
    }
...
void BTree::doadd( Edit* state, void* alloc, void* addr, size_t size)
{
    ...
    Found found(alloc, addr, state);	// result container
